...
export function WordDisplay({ 
  ...
  useEffect(() => {
    ...
    const calculateTextRange = () => {
      ...
      const approximateLineStart = Math.max(0, currentPosition - (currentPosition % charsPerLine));
      ...
      while (testContainer.clientHeight > measuredLineHeight * maxLines && end > currentPosition + 50) {
        end -= 10;
  ...
  // Fixed height for 3 lines based on calculated line height
  const containerHeight = Math.max(200, lineHeight * maxLines + 32); // 32px for padding
  ...
  return (
    ...
    >
      {/* <div className="absolute inset-0 p-4 whitespace-pre-wrap break-words"> */}
      <div className="absolute inset-0 p-4 whitespace-pre-wrap break-words line-clamp-3 overflow-hidden leading-tight">
